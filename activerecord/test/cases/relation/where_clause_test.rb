...
class ActiveRecord::Relation
  class WhereClauseTest < ActiveRecord::TestCase
    test "+ combines two where clauses" do
      first_clause = WhereClause.new(
        [
          predicate_with_binds(table["id"].eq(bind_param), [["id", 1]])
        ]
      )
      second_clause = WhereClause.new(
        [
          predicate_with_binds(table["name"].eq(bind_param), [["name", "Sean"]])
        ]
      )
    ...
    test "+ is associative, but not commutative" do
      a = WhereClause.new([predicate_with_binds("a", [["bind a"]])])
      b = WhereClause.new([predicate_with_binds("b", [["bind b"]])])
      c = WhereClause.new([predicate_with_binds("c", [["bind c"]])])
    ...
    test "merge combines two where clauses" do
      a = WhereClause.new(
        [predicate_with_binds(table["id"].eq(1), [])]
      )
      b = WhereClause.new(
        [predicate_with_binds(table["name"].eq("Sean"), [])]
      )
    ...
    test "merge keeps the right side, when two equality clauses reference the same column" do
      ...
      b = WhereClause.new(
        [predicate_with_binds(table["name"].eq("Jim"), [])]
      )
      expected = WhereClause.new(
        [
          predicate_with_binds(table["id"].eq(1), []),
    ...
    test "merge removes bind parameters matching overlapping equality clauses" do
      a = WhereClause.new(
        [
          predicate_with_binds(table["id"].eq(bind_param), [attribute("id", 1)]),
          predicate_with_binds(table["name"].eq(bind_param), [attribute("name", "Sean")])
        ]
      )
      b = WhereClause.new(
        [
          predicate_with_binds(table["name"].eq(bind_param), [attribute("name", "Jim")])
        ]
      )
      expected = WhereClause.new(
        [
    ...
    test "a clause knows if it is empty" do
      ...
      assert_not WhereClause.new(
        [
          predicate_with_binds("anything", [])
        ]
      ).empty?
    end
    ...
    test "invert replaces each part of the predicate with its inverse" do
      ...
      original = WhereClause.new(
        [
          predicate_with_binds(table["id"].in([1, 2, 3]), []),
          predicate_with_binds(table["id"].eq(1), []),
          predicate_with_binds("sql literal", []),
          predicate_with_binds(random_object, [])
        ]
      )
      expected = WhereClause.new(
        [
          predicate_with_binds(table["id"].not_in([1, 2, 3]), []),
          predicate_with_binds(table["id"].not_eq(1), []),
          predicate_with_binds(Arel::Nodes::Not.new(Arel::Nodes::SqlLiteral.new("sql literal")), []),
          predicate_with_binds(Arel::Nodes::Not.new(random_object), [])
        ]
      )
    ...
    test "accept removes binary predicates referencing a given column" do
      where_clause = WhereClause.new(
        [
          predicate_with_binds(table["id"].in([1, 2, 3]), []),
          predicate_with_binds(table["name"].eq(bind_param), [attribute("name", "Sean")]),
          predicate_with_binds(table["age"].gteq(bind_param), [attribute("age", 30)])
        ]
      )
    ...
    test "ast groups its predicates with AND" do
      ...
      where_clause = WhereClause.new(
        [
          predicate_with_binds(table["id"].in([1, 2, 3]), []),
          predicate_with_binds(table["name"].eq(bind_param), [])
        ]
      )
    ...
    test "ast removes any empty strings" do
      ...
      where_clause_with_empty = WhereClause.new(
        [
          predicate_with_binds(table["id"].in([1, 2, 3]), []),
          predicate_with_binds('', [])
        ]
      )
    ...
    def predicate_with_binds(predicate, binds)
      ActiveRecord::Relation::WhereClause::PredicateWithBinds.new(predicate, binds)
    end
  end
end
